  ...
  function organizeFieldsDirectly(record) {
    const fieldMapping = {
      ...
      'refrigeration_machine': '■ 検討中の冷凍機械'
    };
    ...
    if (record['文字列__お問い合わせ']) {
      ...
      processedFields.add('文字列__お問い合わせ');
    } else {
      output.push(`■ お問い合わせ内容:`);
      processedFields.add('文字列__お問い合わせ');
    }
    ...
    for (const [fieldCode, label] of Object.entries(fieldMapping)) {
      ...
      if (field && field.value !== undefined && field.value !== null && field.value !== '') {
        ...
        if (fieldCode.startsWith('日程希望')) {
          ...
          console.log(`=== 日程希望デバッグ ===`);
          console.log(`フィールドコード: ${fieldCode}`);
          console.log(`日程希望1: ${JSON.stringify(date1)}`);
          console.log(`日程希望2: ${JSON.stringify(date2)}`);
          console.log(`日程希望3: ${JSON.stringify(date3)}`);
          console.log(`値: ${JSON.stringify(value)}`);

          if (date1 === date2 && date2 === date3 && fieldCode !== '日程希望1') {
            console.log(`重複チェックで第${fieldCode.slice(-1)}希望をスキップ`);
